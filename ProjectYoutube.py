...
def get_channel_details(channel_id):
    ...
    playlistitems_response = youtube.playlistItems().list(playlistId=playlist_id,part='contentDetails',maxResults=50).execute()
...
connection = MongoClient("mongodb+srv://thivakaranmnm:0sCtLrACeKogIoQ4@cluster0.u1ol918.mongodb.net/")
...
vid_id = st.sidebar.text_input("Enter YouTube Channel Id")
if vid_id:
    res=get_channel_details(vid_id)
    data=json.dumps(res,indent=4) 
    with open("my_file.json",mode="w+")as ft:
       ft.write(data)
    
    with open("my_file.json",mode="r+")as ft:
       data1=json.load(ft)
       st.write(data1)
